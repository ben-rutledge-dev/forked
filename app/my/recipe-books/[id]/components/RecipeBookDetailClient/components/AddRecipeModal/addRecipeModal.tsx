...
export const AddRecipeModal: React.FC<AddRecipeModalProps> = (props) => {
  ...
  return (
    <div className="p-6">
      <h2 className="text-lg font-semibold text-stone-900 dark:text-stone-100 mb-4">{t('addRecipeHeading')}</h2>
      <TextInput
        type="text"
        value={recipeSearch}
      ...
      <div className="max-h-72 overflow-y-auto space-y-1">
        {addableRecipes.length === 0
          ? (
              <p className="text-center text-stone-400 dark:text-stone-500 text-sm py-6">{t('noRecipesToAdd')}</p>
            )
          : (
              addableRecipes.map(r => (
                <button
                  ...
                  className="w-full flex items-center gap-3 rounded-lg px-3 py-2 text-left hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors"
                >
                  {r.coverImageUrl
                    ? (
                    ...
                    : (
                        <div className="w-10 h-10 rounded-lg bg-stone-100 dark:bg-stone-800 shrink-0" />
                      )}
                  <span className="text-sm font-medium text-stone-900 dark:text-stone-100 line-clamp-1">{r.title}</span>
                </button>
              ))
            )}
      </div>
