  ...
  return (
    <div className="relative">
      ...
      <div
        ref={setNodeRef}
        style={style}
        className="flex items-center gap-1.5 rounded-lg squircle shadow-sm bg-white dark:bg-stone-700 px-1.5 py-1.5 group select-none hover:shadow-md transition-shadow"
      >
        {canEdit && (
          <button
            {...attributes}
            {...listeners}
            className="shrink-0 touch-none cursor-grab active:cursor-grabbing text-stone-300 dark:text-stone-600 hover:text-stone-400 dark:hover:text-stone-500 transition-colors p-0.5"
            tabIndex={-1}
          >
        ...
        {entry.recipe?.id
          ? (
              ...
              >
                ...
                <span className="text-xs font-medium text-stone-800 dark:text-stone-200 truncate min-w-0">
                  {entry.recipe.title}
                </span>
              </Link>
            )
          : (
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <RecipeImage entry={entry} />
                <span className="text-xs font-medium text-stone-800 dark:text-stone-200 truncate min-w-0">—</span>
              </div>
            )}

        {canEdit && (
          <button
            className="shrink-0 cursor-pointer text-stone-300 dark:text-stone-600 hover:text-danger-500 transition-colors opacity-0 group-hover:opacity-100 text-base leading-none p-0.5"
